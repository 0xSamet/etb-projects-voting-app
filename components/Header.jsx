...
import { Header, Button, Divider, Icon, Menu, Popup } from "semantic-ui-react";
...
export default function MyHeader() {
  ...
  const metamaskLoginBtn = (
    <div className="metamask-btn login-btn" onClick={connectMetamask}>
  ...
  const formatWalletAdressForMob = useMemo(() => {
    const wallet = state.user.wallet;
    return (
      wallet.slice(0, 7) +
      "..." +
      wallet.slice(wallet.length - 7, wallet.length)
    );
  }, [state.user.wallet]);

  const formatTokenCount = useMemo(() => {
    const tokenHave = state.user.tokenHave;
    return numeral(tokenHave).format("0,0.00000");
  }, [state.user.tokenHave]);
    ...
    <>
      ...
      <header>
        ...
        <div className="header-right">
            ...
            <Popup
              ...
              trigger={
                <Button
                  ...
                  loading={false}
                  labelPosition="right"
                  className="login-options-trigger-btn"
                >
          ...
          <div
            ...
            style={{ position: isMobileMenuActive ? "fixed" : "static" }}
            onClick={() => setIsMobileMenuActive(!isMobileMenuActive)}
          >
            <Hamburger
              toggled={isMobileMenuActive}
              toggle={setIsMobileMenuActive}
              color={isMobileMenuActive ? "#333" : "#fff"}
              // buttonWidth={30}
            />
          </div>
        </div>
      ...
      >
        <div className="content-wrapper">
          {state.user.loggedIn ? (
            <>
              <Header as={"h5"}>Your Wallet</Header>
              <Divider />
              <Header as={"h5"}>{formatWalletAdressForMob}</Header>
              <Header
                as={"h5"}
                className="token-have-title"
                style={{ marginTop: 10 }}
              >
                Etb Token Have
                <span
              ...
              <Divider />
              <Header as={"h5"}>{formatTokenCount}</Header>
              {logoutBtn}
            </>
          ) : (
            <>
              <Header as={"h5"}>Connect Wallet</Header>
              <Divider />
              {walletConnectLoginBtn}
            </>
          )}
